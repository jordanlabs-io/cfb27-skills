...
3. **Run/pass: VISION IS THE AUTHORITY *among derived lanes* — audio never overrides it (user directive 2026-07-31), but the postgame play-by-play does (rule 3a).** The announcer read is still computed and stored in `play_type_transcript` so conflicts stay visible, but it must not rewrite `play_type`; `assemble.py` resolves `vis_type or tr_type`, and `merge_adjudication.py`'s play_type override must not be run. Vision's known pass-bias is disclosed as a caveat, not "corrected" by commentary.
3a. **Postgame play-by-play is the source of truth for what a play WAS (user directive 2026-08-21).** When the postgame HIGHLIGHTS screens were captured, their per-play rows — "3rd & 1 on RU 0. 1 yard rush by Miles O'Neill for a TD." — state down, distance, ball spot, ball carrier, result and yards. That is proof under the top-line directive, so it outranks the charted read: where the PBP says rush and vision charted pass, **the chart is corrected, not the PBP**. Film-room charting is the FALLBACK for the play record, used where no PBP exists — which is most non-UNC games, since these screens are only captured for the user's own games. Align PBP rows to windows by monotonic sequence alignment within a quarter (down/distance match + clock proximity), never nearest-clock alone: several windows share a clock (PAT / PATGOOD / KICKOFF all print the same time) and a nearest-clock join collapses them onto one row, which measured as a fake 54% dd error rate that rose to 88% once the alignment was fixed. `validate_chart.py`'s "run-labelled but outcome implies pass" check finds these independently — treat a hit there as a reconciliation to apply, not a curiosity. Record every flip in the play's `note` and in the report's Corrections section so a later session can tell a reconciliation from an original read. **What PBP does NOT give you: scheme.** Formation, shell, coverage, leverage, tempo and menu counters exist only in the film lanes, so a game with PBP still needs charting — the PBP just settles run/pass, carrier and yards.
...
4. Coverage shells, routes, motion: vision (post-snap frames / composites), behavior tells per `references/extraction-framework.md`. **Know this lane's ceiling.** On wide broadcast-angle film the secondary is too small to read: measured on UNC-at-Rutgers, `cb_leverage_pre` and `saf_depth_band` came back unknown on 12 of 12 calibration snaps even at native 1568px, and post-snap fields were unknown on ~80% of snaps. Worse, the fields that *were* filled did not hold up — cross-checked against visible play art (rule 1b, the call itself), only 1 of 3 agreed. **A filled coverage field is not evidence it was read.** When counters (rule 1) or play art (rule 1b) are available, they settle coverage; when neither is, say the coverage is unknown rather than shipping a tendency built on it.
...
0. **Preflight — STOP if this fails. Do not start a charting run you cannot clean up after.**
   ...
   python3 "$SK/film-room/scripts/reconcile_film.py" ~/CFB27-film --vault-root "$VAULT"
   python3 "$SK/film-room/scripts/archive_audit.py" ~/CFB27-film --vault-root "$VAULT"
   ```
   ...
   - **`reconcile_film.py` is the deletion authority.** Register every slugged workspace and loose root recording in `operations/film-ingest-ledger.csv`. Treat a game as vault-complete only when the game note exists, the vault CSV exactly matches `plays_charted.csv`, and `chart_validation.json` says pass for that exact chart hash. A Drive receipt or ledger row alone never licenses deletion. Loose media at the film root is always blocked.
   - **`archive_audit.py` non-zero → reconcile/process the backlog first** (`archive_sweep.py`, step 11's automation). The audit PARSES each `drive_upload.json` and the live vault parity state — mere marker existence proved untrustworthy: on 2026-08-19 a saved gws 401 error body was masquerading as an "archived" marker on an 11.6GB game, and half the corpus (~67GB) turned out unarchived while every run's step-0 size check looked at a marker that lied.
...
3.5 **Snap times — run `snap_times.py GAMEDIR` BEFORE frames.py** (calibration 2026-08-21, UNC at Rutgers). CFB 27 stops the play clock at the snap, and the snap is the **start** of the terminal frozen/unreadable block before the clock resets to 40 — not the end of it. Two long-standing bugs both landed frames on the play-call screen instead of the snap: `frames.py`'s motion-onset estimate fired **~4s early on 96% of windows** (on online H2H film the first sustained motion is the play-call UI, not the snap), and `timeline_snaps.py` takes the *end* of the freeze and runs at stage 8, after frames are already cut, so it only ever patched tempo columns. Both are fixed, but run `snap_times.py` anyway: it reads the **rescued** `seg/hud_timeline.csv` (87% readable after the Claude-sheet rescue vs 40% raw), so it beats a fresh in-process OCR pass. Then pass `--snaps seg/snaps.csv` to frames.py; windows with no play-clock snap fall back per-play to the motion estimate, and snaps landing outside their own window are dropped rather than cutting frames from a neighbouring play. Sanity check the printed `median playclock_at_snap` — online H2H film sits ~18-24; a median ≤5 means the lane is reading the wrong band.
...
5. **Keyframes:** `frames.py video.mp4 seg/plays.csv film/ --snaps seg/snaps.csv` (background; parallel by default — `--procs N`, default cpu−2, measured 3.2× on 8 plays; `--procs 1` reproduces the old serial behavior byte-identically) — **presnap_seq 3×2 timestamp-labeled grid (snap−12→−0.5s, the ordered adjustment read)**, preplay 2×2 grid (legacy, still written), presnap, ghost pair, 3x2 strip, result frame, **fullframe.jpg (UNCROPPED snap−1.2s — the v3 full-screen HUD cross-check source: agents read poss/dd/score off the whole scorebug, `hud_conflict` flags disagreements with the machine lane)**, **playart.jpg (uncropped snap−8/−5 pair — play-art preview candidate; agents fill `*_playart_*` only when an overlay is genuinely visible)**, meta.txt. `--plays` takes numbers, ranges, or a mix, and **exits non-zero if any selected play is absent** (it used to silently produce nothing). Snap estimates are clamped into the play window, and windows ≤5s are tagged `snap_unreliable=1 short_window=` — **chart those as `non-play` unless the composites clearly show live football**; their snap cannot be localized and the frames can come from anywhere in the film. Snap-localization caveats: `references/calibration-history.md`.

   **Check the ghost before you trust it — and use `--no-ghost` when the camera pans.** The long-exposure min/max blend assumes a roughly static camera. On broadcast-angle film that pans with the play it stacks the stadium on itself and the composite is noise: on UNC-at-Rutgers a sonnet agent reported "ghost frames corrupted static" on 8 of 12 plays, and every ghost-derived field (`def_safeties_post`, `def_cb_technique`, `def_zone_type`, `rushers`, `def_adjust`, `postsnap_confirms`) came back unknown on 12 of 12. This is NOT a `deshake` problem — regenerating without it produces identical garbage — and it is not specific to one game: ghosts were smeared on all three other films spot-checked (2027 Rutgers-Cincinnati, 2026 UNC-Baylor, 2027 UNC-Vanderbilt). `--no-ghost` emits `snap1..5.jpg` instead: native-res full frames at snap +0.4/+0.9/+1.5/+2.1/+2.7s, stepped through as a flipbook. That set carries strictly more information on panning film and is legible at the API's 1568px cap, where a 3×2 grid cell is not. Open one ghost from the game and look at it before deciding — this costs ten seconds and the failure is obvious to the eye.
6. **Vision charting (Claude primary):** **sonnet** subagents (see the model note at the end of this step), batched ≤12 plays on composites or **≤6 on native-res frame sets**, prompt + superset-JSON schema from `references/charting-prompts.md` (**schema v3** — canonical field/enum list in `references/chart-schema.md` / `scripts/chart_schema.py`) — includes the **ordered pre-play read** (`formation_initial`/`def_shell_initial`/`presnap_adjust`/`adjust_seq`/`def_adjust`: the timestamped presnap_seq cells give initial look → adjustments in sequence → final look, and `postsnap_confirms` cross-checks the final read against ghost/strip), the **v3 tell fields** (CB leverage, safety depth band, trips safety/nickel side, ball hash, QB pre-snap animation, PA/RPO mesh behavior, DL stunt behavior, def_bust, screen_dir), the **full-frame HUD read** (`hud_dd`/`hud_poss`/`hud_score` off fullframe.jpg), and the **play-art read** (playart.jpg, only when the overlay is visible). Paste the ≤10-line condensed tell table from `references/presnap-tells.md` into each batch prompt — tells inform reads, never override observed behavior. **Build every batch prompt with the byte-identical cached prefix** (rubric → 3 key images → game context, per charting-prompts.md "Prompt-cache prefix") — the keys are re-sent ~10×/game otherwise. `prep_batches.py` builds batches + full-res **menu crops** (Lane B; `--tiles` splits each band into two ≤1568px native-res tiles when tile text comes back garbled) + OCR-gated **playart_check crops** (coach re-opening play art pre-snap → `playart_delta`); agents fill `screen_call`/`menu_tiles` from play-call menus. `fanout.py` → legacy blocks + chart_v2 jsonl → `merge_results.py` + `merge_v2_local.py`. Tag provenance `v2_src=claude-haiku-4.5-frames/<date>`. Lane A upgrade: Gemini cloud respot on the public YouTube copy — `references/my-games-twitch.md`. For the post-snap coverage fields specifically, the charting agent is given `references/coverage-recognition.md` alongside `charting-prompts.md` — the decision-tree reference for `def_coverage`/`def_shell_pre`/`def_cb_technique`/`def_zone_type`/`def_safeties_post`, including the NFL BDB2026 priors section for setting a prior when frames are ambiguous.

   **Model: sonnet, not haiku (calibration 2026-08-21).** Controlled run — 12 hand-picked discriminating plays, identical frames, byte-identical prompt, only the model varying:

   | | sonnet | haiku |
   | --- | --- | --- |
   | run/pass vs the postgame PBP key | 75% | 42% |
   | `hud_dd` transcription | 92% | 91% |
   | scheme fields *filled* | 38% | 86% |
   | `def_coverage` ever named | 1/12 | 0/12 |

   Read the fill row the right way round: haiku filled **more** and agreed with sonnet on **almost none** of it (formation 0/12, `def_shell_pre` 2/12, `def_safeties_post` 1/12). They are not disagreeing — one is inventing. Upgrading the frames moved haiku's HUD reading 73%→91% and its run/pass judgement 50%→**42%**: legibility fixes what a model *reads*, nothing fixes what it *judges*. This is the mechanism `frame_quality.py` already documents ("a vision model does not refuse an illegible frame, it transcribes a coin flip — measured 52% correct"), and it is why haiku charts always needed a rescue pass: the rescue was laundering confabulated scheme fields. Transcription-only work (menu tiles, HUD sheets, Lane C) is still fine on haiku — it is judgement that fails. **Consequence for the vault: behaviour-derived coverage reads in dossiers charted by haiku off smeared ghosts should be treated as suspect; call-sheet counter sections are unaffected.** Tag provenance accordingly (`v2_src=claude-sonnet-5-frames/<date>`).

   **Hand the agents the menu crops.** `prep_batches.py --tiles` writes `menuN_L/_R.jpg` per play, and they are useless sitting on disk: on UNC-at-Rutgers they were omitted from the frame list and the whole game charted with `menu_tiles: []` — 0 of 123 plays — which silently removed rule 1 (counter truth) from the ingest and left `menu_book.py` with nothing to book. If a charting batch's frame list does not name the menu crops, the call-sheet book will be empty and no error will tell you. A separate transcription-only pass can recover it after the fact (441 tiles, 103 plays booked) but that is a second full sweep of the film — cheaper to get right the first time.

   **Whose menus are they?** On the user's OWN screen every menu is the USER's call sheet, not the opponent's. That film updates `call-sheets/<user-team>.csv` and adds **nothing** to the opponent's counter book — his counters only come from film of HIS screen. Say so explicitly in the dossier rather than letting a reader assume the counters are his.
...
11. **Archive + cleanup:** `python3 "$SK/film-room/scripts/archive_sweep.py" ~/CFB27-film --vault-root "$VAULT" [--only SLUG] [--dry-run]` — it may upload originals for preservation, but it deletes NOTHING unless live reconciliation reports `delete_ready`. For games that means a passing hash-bound validation receipt + game note + byte-identical vault CSV; for Lane C it means a capture index + coverage ledger + zero-untranscribed proof. No force-delete bypass exists. After the vault gate passes, it verifies Drive `md5Checksum` against local `md5 -q`, writes a uniform `drive_upload.json`, and deletes the bulky regenerables (originals' local copies, `video.mp4`, `audio.wav`, `retranscode/`, `seg/hud_frames/`). It runs `gws` from inside each game dir (gws rejects `--upload` paths outside the cwd) and treats `video.mp4` as an ORIGINAL, not a regenerable, when no ScreenRecording/`.mov` capture exists (data-dump dirs). KEEP list unchanged: `plays_charted.csv`, `transcript.json`, `seg/*.csv`, rescue JSONs, `film/` composites (small, needed for re-charting). Finish with both `reconcile_film.py` and `archive_audit.py` → both must exit 0.
...
2. **Classify + transcribe (≤12 screens/batch):** each screen gets a category **and a layout variant** (screen title + tab/chip state) + **verbatim transcription** — numbers exactly as displayed, garbles quoted with `[sic]`, never inferred or totaled (agent contracts: `references/dump-prompts/`). Emit one JSON object per screen to `OUTDIR/menu-intel.jsonl`.
   ...
   - **Never let the model count glyphs.** Star ratings go through `stars_cv.py` (6/6 vs the model's 3/6 — and an explicit prompt fix made the model *worse*, 2/6). Same for red injured rows and rating arrows: transcribe the observation, let a script decode it. **The green up-arrow on a roster/depth-chart rating is a coach-ability BOOST indicator, not progression** (user correction 2026-08-21, dynasty-hq dictionary §5.3): the roster/depth-chart number is the *effective* (boosted) value, the player card shows the *base* value. Both are correct — commit both, never reconcile them, and never flag the difference as a conflict. Cross-check: arrow present ⇒ effective > base; arrow absent ⇒ equal; violation ⇒ re-read.
...
3. **Structured merge → vault targets** (all tables must stay column-consistent for the verifier):
   ...
   - roster screens → the fields CFB Labs can't supply (archetype, dev trait, redshirt, injury status) into the dynasty roster files. **Rating keys carry their basis, never bare** (`ovr_effective` from roster/depth-chart screens, `ovr_base` from player cards — dynasty-hq `dictionary/fields.csv` rating_basis rule); a capture with no player-card screens holds boosted values only — flag it "effective-only coverage" in the capture index;
...
| `query_plays.py` | `{teams,disguise,audit-coverage,blitz,search} [--dynasty-dir] [--team] [--game] [--json]` | both / 9 situational query engine |
...
| `reconcile_film.py` | `[FILM_ROOT] --vault-root VAULT [--write-ledger] [--json]` | both / vault-parity + deletion gate |
